...
void *AtenderCliente (void *indice)
{
	...
	while (fin == 0)
	{
		...
		else if (codigo == 5)
		{
			...
			err = CreaPartida(&listaPartidas, id_partida);
			...
			sprintf(respuesta, "5/%d_%s", num_jugadores, invitados);
		...
		else if (codigo == 6)
		{
			...
			printf("444444 num_j: %d\n", num_jugadores);
			
			p = strtok(NULL, "/");
			strcpy(usuario, p);
			printf("555555 user: %s\n", usuario);
			
			p = strtok(NULL, "/");
			aceptado = atoi(p);
			
			printf("666666 aceptado: %d  \n", aceptado);
			
			p = strtok(NULL, "/");
			strcpy(jugadores, p);
			printf("111111 jugadores: %s\n", jugadores);
			...
			if (contestados == num_jugadores)
			{
				...
				printf("88888888 \n");
				sprintf(respuesta, "6/%d", devolver);
	...
	int main(int argc, char *argv[])
	{
		...
		serv_adr.sin_port = htons(9100); //50001
